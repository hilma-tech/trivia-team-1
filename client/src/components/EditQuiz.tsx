...
const EditQuiz: FC = () => {
  ...
  const addQuestion = () => {
    if (questions.length < 10) {
      ...
      setQuestions((prev) => {
        ...
        return [
          ...prev,
          { questionId: lastQuestion.questionId + 1, answers: ["", ""], questionTitle: "" },
        ];
      });
    }
  };
  ...
  function handleDragEnd(result: any) {
    ...
    if (editItem.questionId === reorderedItem.questionId) {
      ...
      items.splice(result.destination.index, 0, reorderedItem);
      setQuestions(items);
      const editQuestionIndex = items.findIndex(
        (question) => question.questionId === editItem.questionId
      );
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setQuestionDetails((prev) => {
      return id === "quizInputName"
        ? { ...prev, quizName: value }
        : { ...prev, quizDescription: value };
    });
  };
            ...
            <div className="top-buttons-container">
              ...
              <div className="top-left-btn">
                <button className="link-btn">
                  <img className="link-btn-svg" src={LinkBtn} />
                </button>
                            ...
                            <AddQuestionBox
                              setCurrentQuestion={(q) => {
                                setQuestions((prev) => {
                                  return [
                                    ...prev.slice(0, index),
                                    typeof q === "function" ? q(question) : q,
                                    ...prev.slice(index + 1),
                                  ];
                                });
                              }}
